...
def make_new_student(first_name, last_name, github):
    ...
    success_message = "Successfully added student: {first} {last}".format(
...
def make_new_project(title, description, max_grade):
    ...
    Given a title, description, max_grade, add project to the
    database.
    """
...
def assign_grade(github, title, grade):
    ...
    # check if project already graded
    db_cursor = db.session.execute(QUERY_SELECT, {'github': github,
                                                    'title': title})
    ...
    if len(db_cursor.fetchall()) != 0:
        ...
        db_cursor = db.session.execute(QUERY_INSERT, {'github': github,
                                           'title': title,
                                           'grade': grade})
